...
function legacyPetReceipts(entries, componentStamp, petsDir, slugs, home) {
  const stampedSlugs = componentStamp?.slugs
  const stampedSlugsValid = Array.isArray(stampedSlugs) &&
    stampedSlugs.every(slug => typeof slug === 'string' && slug.length > 0)
  const stampedKeys = stampedSlugsValid ? stampedSlugs.map(slug => slug) : []
  const expectedKeys = new Set(slugs)
  if (!stampedSlugsValid || stampedKeys.length !== new Set(stampedKeys).size ||
      stampedKeys.length !== expectedKeys.size || stampedKeys.some(slug => !expectedKeys.has(slug))) {
    throw new Error('The legacy pet ownership stamp does not identify this installed pet set.')
  }

  const receipts = new Map()
  for (const entry of [...entries].reverse()) {
    if (entry.type !== 'pet' || !expectedKeys.has(entry.slug) || receipts.has(entry.slug)) continue
    receipts.set(entry.slug, entry)
  }
  if (receipts.size !== expectedKeys.size) {
    throw new Error('The legacy pet ownership stamp has incomplete pet receipts.')
  }

  for (const slug of slugs) {
    const receipt = receipts.get(slug)
    const expectedDirectory = join(petsDir, slug)
    assertSafeManagedPath(home, expectedDirectory, 'legacy pet directory')
    if (!receipt || typeof receipt.dir !== 'string') {
      throw new Error('The legacy pet ownership stamp has incomplete pet receipts.')
    }
    assertSafeManagedPath(home, receipt.dir, 'legacy pet directory')
    if (pathKey(receipt.dir) !== pathKey(expectedDirectory)) {
      throw new Error(`Legacy pet receipt does not match the selected pets directory: ${slug}`)
    }
    if (receipt.preExisting) {
...
export function installPets(bundledPetsDir, petsDir, options = {}) {
  ...
  const legacyBySlug = !activeTransaction && componentStamp
    ? legacyPetReceipts(entries, componentStamp, petsDir, slugs, home)
    : new Map()
  ...
  for (const slug of slugs) {
    ...
    for (const sourceRelative of listFiles(sourceRoot)) {
      ...
      const adoptLegacy = Boolean(!activeTransaction && componentStamp && legacy &&
        !legacy.preExisting && currentHash === sourceHash)
      const managedBefore = Boolean(priorReceipt || adoptLegacy)

      if (!activeTransaction && componentStamp && !managedBefore && currentExists) {
