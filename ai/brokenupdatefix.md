...
| **Status bar spills under the left sidebar** | Cosmetic: Hermes's own stock footer is full-width; the gold tape floats above it, centred on the chat pane. Only a concern if the tape itself overlaps the sidebar. | Tape offset lives in `TelemetryTape` (`left: calc(50% + var(--pane-chat-sidebar-width)/2)`). If it's off, that calc — not the stock footer — is the thing to check. |
