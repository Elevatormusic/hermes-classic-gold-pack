...
| **Whole status bar missing** (no footer at all — not just the gold tape) even though the statusbar tier is applied and built | Newer Hermes made the status bar **opt-in, hidden by default** (`hermes.desktop.statusbarVisible`, default `false`); the tape renders inside the bar, so it's unmounted with it. | Press **Ctrl/Cmd+Shift+S** in Hermes ("Toggle status bar" — persists), or re-run `node theme/apply-theme.mjs` (the theme snippet enables the bar when no preference is set). |
