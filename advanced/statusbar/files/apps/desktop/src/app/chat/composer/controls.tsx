...
export function ComposerControls({
  ...
  return (
    <div className="ml-auto flex shrink-0 items-center gap-(--composer-control-gap)">
      {/* Classic Gold: the status tape carries its own model selector, so the
          composer's model pill is redundant whenever the tape is on-screen. It's
          hidden via CSS at the tape's >=880px breakpoint (see styles.css) and kept
          as a fallback on narrow windows / pop-outs where the tape isn't shown. */}
      <span className="contents" data-slot="composer-model-pill">
