...
import { $currentProvider } from '@/store/session'
...
export function useStatusSnapshot(gatewayState: string | undefined, requestGateway: GatewayRequester) {
  // Evaluate readiness for the ACTUALLY-SELECTED provider, not a default. Without
  // this, switching to a working local model (e.g. custom LM Studio endpoint)
  // evaluated some other default provider and showed a false "needs setup".
        ...
        const [statusResult, inferenceResult] = await Promise.allSettled([
          ...
          gatewayState === 'open'
            ? evaluateRuntimeReadiness(requestGateway, {
                requestedProvider: currentProvider.trim() || undefined
              })
  ...
  }, [gatewayState, requestGateway, currentProvider])
