...
export function ContextUsagePanel({ currentUsage, requestGateway, sessionId }: ContextUsagePanelProps) {
  ...
  const contextUsed = Math.max(breakdown?.context_used ?? 0, currentUsage.context_used ?? 0)
  const reportedPercent = breakdown?.context_percent ?? currentUsage.context_percent ?? 0
  const contextPercent =
    contextMax > 0 ? Math.max(0, Math.min(100, Math.round((contextUsed / contextMax) * 100))) : reportedPercent
