 ...
 */
function recordInstall(tier, head, via, rels, additiveOnly, baseline) {
  try {
    const home = resolveHermesHome({})
    if (!home) return
    recordApplied(home, tier, { via, agentHead: head, baseline: baseline.id }, { version: packVersion(), base: baseline.commit, agentHead: head })
...
function verifyBuild(repo, buildStartMs, tier, baseline) {
  ...
  if (tier === 'statusbar') {
    try {
      ...
      const mainRel = baseline.electronExt === 'ts' ? 'electron/main.ts' : 'electron/main.cjs'
      const main = join(repo, 'apps', 'desktop', mainRel)
      ...
      if (usesBridge && !hasHandler) {
        console.warn(`! statusbar reads window.hermesDesktop.getSystemResources but ${mainRel} has no`)
...
export function applyTier({ scriptDir, patchName, tier, label }) {
  ...
  if (!sel.baseline) {
    console.error(
      `✗ No Classic Gold baseline matches your Hermes ` +
        `(HEAD ${String(head).slice(0, 7)}, app ${sel.appVersion || '?'}, electron ${sel.electronExt || '?'}).`
    )
    console.error('  Reconcile per ai/repair.md, then commit the result as a new')
    console.error('  advanced/<tier>/baselines/<appVersion>-<shortsha>/ entry + a baselines.json row.')
    return 1
  }
  const baseline = sel.baseline
  const baselineDir = join(scriptDir, 'baselines', baseline.id)
  if (head && head !== baseline.commit) {
    console.warn(
      `! HEAD ${head.slice(0, 7)} != baseline ${baseline.id} commit ${baseline.commit.slice(0, 7)} ` +
        `(matched via ${sel.matchType}) — patch may not apply; will fall back to file copy.`
    )
  } else {
    console.log(`• Baseline: ${baseline.id} (matched via ${sel.matchType}).`)
  }
  ...
  const fallbackRoot = join(baselineDir, 'files')
  ...
  const patch = join(baselineDir, patchName)
  const r = spawnSync('git', ['-C', repo, 'apply', '--3way', '--whitespace=nowarn', patch], { stdio: 'inherit' })
  if (r.status !== 0) {
    const diverged = Boolean(head && head !== baseline.commit)
    ...
    for (const rel of rels) {
      ...
      if (additive) {
        ...
        const ap = spawnSync(
          'git',
          ['-C', repo, 'apply', '--3way', '--whitespace=nowarn', join(baselineDir, additive)],
          { stdio: 'inherit' }
        )
        if (ap.status !== 0) {
    ...
    if (unresolved.length) {
      ...
      console.error(
        `\n✗ ${unresolved.length} file(s) can't be applied cleanly on your Hermes version ` +
          `(HEAD ${String(head).slice(0, 7)} != baseline ${baseline.id} commit ${baseline.commit.slice(0, 7)}):`
      )
  ...
  recordInstall(tier, head, r.status === 0 ? 'patch' : 'copy', rels, ADDITIVE_ONLY, baseline)
  ...
  if (args.build) {
    ...
    verifyBuild(repo, buildStart, tier, baseline)
    ...
    console.log('  * To update later, just ASK YOUR AI ASSISTANT: "update my Hermes and')
    console.log('    re-apply Classic Gold" (point it at ai/update.md) - it re-applies AND')
    console.log('    self-heals. Prefer that over the in-app Update button.')
    console.log('    (Comfortable in a terminal? node update-hermes.mjs does the same.)')
