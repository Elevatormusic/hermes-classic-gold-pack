...
// Classic Gold status bar: system RAM (always) + GPU VRAM (nvidia-smi, best-effort).
// Returns bytes; the status-bar tape formats to GB. vram is null when no NVIDIA GPU.
ipcMain.handle('hermes:system-resources', async () => {
  const total = os.totalmem()
  const ram = { total, used: total - os.freemem() }
  let vram: { total: number; used: number } | null = null
  try {
    ...
    const row = out
      ...
      .find(Boolean)
    if (row) {
      const [t, u] = row.split(',').map(s => Number(s.trim()) * 1024 * 1024) // MiB -> bytes
      if (Number.isFinite(t) && Number.isFinite(u) && t > 0) vram = { total: t, used: u }
    }
  } catch {
    // no nvidia-smi / no NVIDIA GPU — leave vram null (RAM still shows).
  }
  return { ram, vram }
})
